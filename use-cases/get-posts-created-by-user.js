/**
 * Get all posts created by user from DB
 * @param {postsDb, postPhotosDb}
 */
const makeGetPostsCreatedByUser = ({ postsDb, postPhotosDb, makeTransaction }) => {
  return async ({ userId, after, limit }) => {
    const result = await makeTransaction(async transaction => {
      const { posts, postsCount, since } = await postsDb.findPostsCreatedByUser({ userId, after, limit }, transaction)
...
export default makeGetPostsCreatedByUser
