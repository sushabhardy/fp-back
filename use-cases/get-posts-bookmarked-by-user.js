/**
 * Get all posts bookmarked by user from DB
 * @param {postsDb, postPhotosDb}
 */
const makeGetPostsBookmarkedByUser = ({ postsDb, postPhotosDb, makeTransaction }) => {
  return async ({ userId, after, limit }) => {
    const result = await makeTransaction(async transaction => {
      const { posts, postsCount, since } = await postsDb.findPostsBookmarkedByUser({ userId, after, limit }, transaction)
...
export default makeGetPostsBookmarkedByUser
